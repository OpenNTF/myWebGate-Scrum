...
public class FileService {
	private static FileService m_Service;
	...
	public List<FileHelper> getValue(Object obj, String strFieldName,
			String strCcId, HashMap<String, FileHelper> valueFields) {
	...
	public void uploadFile(UploadedFile file, Object obj, String strFieldName,
			String strCcId, String strNewId, List<FileHelper> objFiles,
			HashMap<String, FileHelper> fileList) {
		try {
			Method mt = null;
			...
			if (fhCurrent != null) {
				FileHelper fhNew = FileService.getInstance().addFile(fhCurrent,
						file, strNewId);
				objFiles.add(fhNew);
	...
	public FileHelper addFile(FileHelper fh, UploadedFile file, String strNewId) {
		try {
			...
			EmbeddedObject em = rt.embedObject(EmbeddedObject.EMBED_ATTACHMENT,
			...
			fhNew.setFileType(ComponentSessionFacade.get().getMimeTypes()
					.getContentType(FNew));
